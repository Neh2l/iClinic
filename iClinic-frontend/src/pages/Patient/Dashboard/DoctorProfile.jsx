...
function DoctorProfile() {
  ...
  useEffect(() => {
    const checkSubscription = async () => {
      ...
      try {
        ...
        if (response.ok && data.data) setIsSubscribed(data.data.hasAccess);
  ...
  const selectPlan = (plan) => {
    navigate('/checkout', { state: { plan, doctor } });
  };

  const handleMessage = () => {
    if (isSubscribed) {
      navigate('/patient/messages', { state: { selectedDoctor: doctor } });
    }
  };
    ...
    <PatientLayout>
      <div className={style.doctorProfile}>
        ...
        <div className={style.profileHeader}>
              ...
              {checkingSubscription ? (
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                  <button
                    ...
                    title={!isSubscribed ? 'Subscribe to a plan to message this doctor' : ''}
                  >
                    Message
                  </button>
                  ...
                  <button
                    ...
                    title={!isSubscribed ? 'Subscribe to a plan to book appointments' : ''}
                  >
                    Book Appointment
                  </button>
                </div>
          ...
          <div className={style.profileInfo}>
            ...
            <div className={style.experience}>
              <h5>Experience</h5>
              <p>{doctor.experience || '10'} years / {doctor.consultations || '1000+'} consultations</p>
            </div>
          </div>
        </div>
        ...
        <div className={style.plansSection}>
          ...
          {isSubscribed && !checkingSubscription && (
            <p style={{ color: 'green', marginBottom: '10px' }}>✓ You are subscribed to this doctor</p>
          )}
            ...
            {plans.map((plan, index) => {
              ...
              return (
                <div
                  ...
                  style={{
                    background: isLastPlan
                      ? 'linear-gradient(135deg, #00c6ff, #015D82)'
                      : '#fff',
                    color: isLastPlan ? '#fff' : '#fff',
                    border: isLastPlan ? 'none' : '1px solid #ddd'
                  }}
                >
                  <h5>{plan.name}</h5>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      cursor: isSubscribed || checkingSubscription ? 'not-allowed' : 'pointer'
                    }}
                  >
                    {checkingSubscription
                      ? 'Loading...'
        ...
        <div className={style.reviewsSection}>
          ...
          <div className={style.reviewsContainer}>
            <div className={style.reviewCard}>
              ...
              <h5>Quick & Helpful</h5>
              <p>I got medical advice within minutes. The online chat made me feel safe and supported.</p>
              <div className={style.reviewer}>
                <img src="/image 4.png" alt="Ahmed R" className={style.reviewerImg} />
                <div>
            ...
            <div className={style.reviewCard}>
              ...
              <p>The consultation was smooth and the doctor explained everything clearly. I'll definitely use it again!</p>
              <div className={style.reviewer}>
                <img src="/image 4.png" alt="Lina K" className={style.reviewerImg} />
                <div>
        ...
        {showBookingModal && (
          ...
          >
            ...
            >
              <h3 style={{ marginTop: 0 }}>Book Appointment with Dr. {doctor.fullName}</h3>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', maxHeight: '300px', overflow: 'auto', padding: '5px' }}>
                  {timeSlots.map((time) => (
                    <button
                      key={time}
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
                <button
                  ...
                  style={{
                    flex: 1,
                    padding: '12px',
                    backgroundColor: bookingLoading || !selectedDate || !selectedTime ? '#ccc' : '#28a745',
                    color: '#fff',
                    border: 'none',
                    borderRadius: '4px',
                    cursor: bookingLoading || !selectedDate || !selectedTime ? 'not-allowed' : 'pointer',
                    fontWeight: 'bold'
                  }}
                >
                ...
                <button
                  onClick={() => { setShowBookingModal(false); setSelectedDate(''); setSelectedTime(''); setBookingError(''); setBookingSuccess(''); }}
